...
public class EventServiceImpl implements EventService {
    ...
    public EventFullDto addEvent(Long userId, NewEventDto dto) {
        Category category = categoryRepository.findById(dto.getCategory()).orElseThrow(() -> new NotFoundException(
                String.format("Добавление события. Категория с ID=%s не найдена.", dto.getCategory())));

        User initiator = userRepository.findById(userId).orElseThrow(() -> new NotFoundException(
                String.format("Добавление события. Пользователь с ID=%s не найден", userId)));
    ...
    public EventFullDto getEventById(Long userId, Long eventId) {
        Event event = eventRepository.findById(eventId).orElseThrow(() -> new NotFoundException(
                String.format("Получения данных о событии. Событие с ID=%s не найдено.", eventId)));
    ...
    public EventFullDto patchEventById(Long userId, Long eventId, UpdateEventUserRequest dto) {
        Event oldEvent = eventRepository.findById(eventId).orElseThrow(() -> new NotFoundException(
                String.format("Обновление данных события. Событие с ID=%s не найдено.", eventId)));
        ...
        if (dto.getCategory() != null) {
            Category category = categoryRepository.findById(dto.getCategory()).orElseThrow(() -> new NotFoundException(
                    String.format("Обновление данных события. Категория с ID=%s не найдена.", dto.getCategory())));
    ...
    public List<ParticipationRequestDto> getRequestsOfEvent(Long userId, Long eventId) {
        Event event = eventRepository.findById(eventId).orElseThrow(() -> new NotFoundException(
                String.format("Получение запросов на участие в событии. Событие с ID=%s не найдено", eventId)));
        if (!event.getInitiator().getId().equals(userId)) {
    ...
    public EventRequestStatusUpdateResult patchRequestsStatusOfEvent(Long userId, Long eventId,
        ...
        if (dto.getRequestIds().size() != requests.size()) {
            if (participantLimit.equals(approvedRequestsCount) && participantLimit > 0) {
                ...
                throw new CreationRulesException(String.format("Достигнут лимит одобренных заявок в событии с ID=%s. ", eventId));
            }
        }
    ...
    public EventFullDto patchEventByIdByAdmin(Long eventId, UpdateEventAdminRequest dto) {
        Event oldEvent = eventRepository.findById(eventId).orElseThrow(() -> new NotFoundException(
                String.format("Уровень Admin. Обновление данных события. Событие с ID=%s не найдено.", eventId)));
        ...
        if (dto.getCategory() != null) {
            Category category = categoryRepository.findById(dto.getCategory()).orElseThrow(() -> new NotFoundException(
                    String.format("Обновление данных события администратором." +
                            "Категория с ID=%s не найдена.", dto.getCategory())));
    ...
    public EventFullDto getEventByIdByPublicRequest(Long eventId) {
        Event event = eventRepository.findById(eventId).orElseThrow(() -> new NotFoundException(
                String.format("Публичный запрос на получение данных о событии. Событие с ID=%s не найдено.", eventId)));
